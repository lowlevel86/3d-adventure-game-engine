                   ...
                   PSTR szCmdLine, int iCmdShow)
{
   ...
   hwnd = CreateWindow(szAppName, TEXT("3d Mesh"),
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
   ...
   if (WM_KEYDOWN == message)
   {
      ...
      i = 0;
      while (i < sizeof(mesh_POINT)/sizeof(float))
      {
         lrRot(&mesh_POINT[i], &mesh_POINT[i+1], &mesh_POINT[i+2], xMove);
         tbRot(&mesh_POINT[i], &mesh_POINT[i+1], &mesh_POINT[i+2], yMove);
         ccRot(&mesh_POINT[i], &mesh_POINT[i+1], &mesh_POINT[i+2], zMove);
         i = i + 3;
      }
...
void drawScene(HWND hwnd)
{
   ...
   for (y = 0; y < yWin; y++)
   for (x = 0; x < xWin; x++)
   scrnBuff[x][y] = 0xFFFFFF;
   
   
   lineCnt = 0;
   for (i = 0; i < sizeof(mesh_COLOR)/sizeof(int); i++)
   {
      ...
      if (sidesNum == 2)
      {
         ...
         line(mesh_POINT[pt1]*30, mesh_POINT[pt1+1]*30, mesh_POINT[pt1+2]*30,
              mesh_POINT[pt2]*30, mesh_POINT[pt2+1]*30, mesh_POINT[pt2+2]*30, mesh_COLOR[i]);
      ...
      if (sidesNum == 3)
      {
         ...
         line(mesh_POINT[pt2]*30, mesh_POINT[pt2+1]*30, mesh_POINT[pt2+2]*30,
              mesh_POINT[pt3]*30, mesh_POINT[pt3+1]*30, mesh_POINT[pt3+2]*30, mesh_COLOR[i]);
         
         line(mesh_POINT[pt3]*30, mesh_POINT[pt3+1]*30, mesh_POINT[pt3+2]*30,
              mesh_POINT[pt1]*30, mesh_POINT[pt1+1]*30, mesh_POINT[pt1+2]*30, mesh_COLOR[i]);
      ...
      if (sidesNum == 4)
      {
         ...
         line(mesh_POINT[pt3]*30, mesh_POINT[pt3+1]*30, mesh_POINT[pt3+2]*30,
              mesh_POINT[pt4]*30, mesh_POINT[pt4+1]*30, mesh_POINT[pt4+2]*30, mesh_COLOR[i]);
         
         line(mesh_POINT[pt4]*30, mesh_POINT[pt4+1]*30, mesh_POINT[pt4+2]*30,
              mesh_POINT[pt1]*30, mesh_POINT[pt1+1]*30, mesh_POINT[pt1+2]*30, mesh_COLOR[i]);
